 ...
 */
void
regionview_paint_rect(GdkDrawable *draw, GdkGC *gc, VipsRect *r)
{
	gdk_draw_rectangle(draw, gc, FALSE,
		r->left, r->top,
		VIPS_MAX(0, r->width - 1), VIPS_MAX(0, r->height - 1));
}

/* Paint a thick rectangle.
 */
void
regionview_paint_rect_thick(GdkDrawable *draw, GdkGC *gc, VipsRect *r, int n)
{
	VipsRect our_r;
	int i;
	...
	for (i = 0; i < n; i++) {
		...
		vips_rect_marginadjust(&our_r, 1);
	}
}
 ...
 */
static void
regionview_paint_rect_3d(GdkDrawable *draw, GdkGC *tl, GdkGC *br, VipsRect *r)
{
	/* Bottom and right.
	 */
	gdk_draw_line(draw, br,
		VIPS_RECT_RIGHT(r) - 1, r->top,
		VIPS_RECT_RIGHT(r) - 1, VIPS_RECT_BOTTOM(r) - 1);
	gdk_draw_line(draw, br,
		VIPS_RECT_RIGHT(r) - 1, VIPS_RECT_BOTTOM(r) - 1,
		r->left, VIPS_RECT_BOTTOM(r) - 1);

	/* Top and left.
	 */
	gdk_draw_line(draw, tl,
		r->left, VIPS_RECT_BOTTOM(r) - 1, r->left, r->top);
	gdk_draw_line(draw, tl,
		r->left, r->top, VIPS_RECT_RIGHT(r) - 1, r->top);
}
	...
	VipsRect *r, gboolean locked)
{
	...
	vips_rect_marginadjust(&our_r, n);
	...
	if (!locked && r->width > 20) {
		...
		regionview_paint_vtick(draw, tl, br,
			r->left + 10, r->top - 1, n);
		regionview_paint_vtick(draw, tl, br,
			VIPS_RECT_RIGHT(r) - 11, r->top - 1, n);

		/* Bottom edge.
		 */
		regionview_paint_vtick(draw, tl, br,
			r->left + 10, VIPS_RECT_BOTTOM(r) + n + 1, n);
		regionview_paint_vtick(draw, tl, br,
			VIPS_RECT_RIGHT(r) - 11, VIPS_RECT_BOTTOM(r) + n + 1,
			n);
	}
	...
	if (!locked && r->height > 20) {
		...
		regionview_paint_htick(draw, tl, br,
			r->left - 1, r->top + 10, n);
		regionview_paint_htick(draw, tl, br,
			r->left - 1, VIPS_RECT_BOTTOM(r) - 12, n);

		/* Right edge.
		 */
		regionview_paint_htick(draw, tl, br,
			VIPS_RECT_RIGHT(r) + n + 1, r->top + 10, n);
		regionview_paint_htick(draw, tl, br,
			VIPS_RECT_RIGHT(r) + n + 1, VIPS_RECT_BOTTOM(r) - 12,
			n);
	}
}
...
regionview_paint_label(Regionview *regionview, GdkDrawable *draw,
	VipsRect *r, int ascent, const char *txt)
{
 ...
 */
static void
regionview_paint_arrow(GdkDrawable *draw, GdkGC *fg, int off, VipsRect *r)
{
	...
	gdk_draw_line(draw, fg,
		r->left, r->top, VIPS_RECT_RIGHT(r), VIPS_RECT_BOTTOM(r));
 ...
 */
static void
regionview_paint_box(GdkDrawable *draw, GdkGC *fg, int off, VipsRect *r)
{
	static gint8 dash_list[] = { 10, 10 };
	...
	gdk_draw_line(draw, fg,
		r->left, r->top,
		VIPS_RECT_RIGHT(r), r->top);
	gdk_draw_line(draw, fg,
		VIPS_RECT_RIGHT(r), r->top,
		VIPS_RECT_RIGHT(r), VIPS_RECT_BOTTOM(r));
	gdk_draw_line(draw, fg,
		VIPS_RECT_RIGHT(r), VIPS_RECT_BOTTOM(r),
		r->left, VIPS_RECT_BOTTOM(r));
	gdk_draw_line(draw, fg,
		r->left, VIPS_RECT_BOTTOM(r),
		r->left, r->top);
	gdk_gc_set_line_attributes(fg, 0,
...
regionview_border_foreach(Regionview *regionview,
	VipsRect *border, regionview_rect_fn fn, void *data)
{
	const int n = regionview_border_width + 2;
	...
	area.top = VIPS_RECT_BOTTOM(border);
	if ((res = fn(regionview, &area, data)))
		return res;
	...
	area.left = VIPS_RECT_RIGHT(border);
	if ((res = fn(regionview, &area, data)))
		return res;
 ...
 */
static void *
regionview_queue_draw_area(Regionview *regionview, VipsRect *area, void *dummy)
{
#ifdef DEBUG_PAINT
...
regionview_queue_draw(Regionview *regionview)
{
	...
	case REGIONVIEW_ARROW:
		...
		vips_rect_normalise(&dr);
		vips_rect_marginadjust(&dr, 2);
...
regionview_paint(Regionview *regionview)
{
	...
	case REGIONVIEW_ARROW:
		...
		regionview_paint_crosshair(draw, tl, bg, br,
			VIPS_RECT_RIGHT(&dr), VIPS_RECT_BOTTOM(&dr));
		break;
...
regionview_dispose(GtkObject *object)
{
	...
	VIPS_FREEF(iwindow_cursor_context_destroy, regionview->cntxt);
	...
	if (regionview->ip) {
		...
		regionview->ip->regionviews =
			g_slist_remove(regionview->ip->regionviews, regionview);
...
regionview_setup(Regionview *regionview,
	Classmodel *classmodel, VipsRect *model_area, Imagepresent *ip)
{
	iWindow *iwnd;
...
Regionview *
regionview_new(Classmodel *classmodel, VipsRect *model_area, Imagepresent *ip)
{
