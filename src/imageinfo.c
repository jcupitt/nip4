...
imageinfo_finalize(GObject *gobject)
{
	...
	VIPS_FREEF(im_close, imageinfo->im);
	VIPS_FREEF(im_close, imageinfo->mapped_im);
	VIPS_FREEF(im_close, imageinfo->identity_lut);
...
imageinfo_proxy_add(Imageinfo *imageinfo)
{
	...
	if (!(imageinfo->proxy = VIPS_NEW(imageinfo->im, Imageinfoproxy)))
		if (!(imageinfo->proxy = VIPS_NEW(NULL, Imageinfoproxy)))
			return;
...
#define PRINT_FLOAT(T, I) vips_buf_appendg(buf, ((T *) p)[I]);
	...
	for (i = 0; i < im->Bands; i++) {
		...
		switch (im->BandFmt) {
		case VIPS_BANDFMT_UCHAR:
			PRINT_INT(unsigned char, i);
			break;

		case VIPS_BANDFMT_CHAR:
			PRINT_INT(char, i);
			break;

		case VIPS_BANDFMT_USHORT:
			PRINT_INT(unsigned short, i);
			break;

		case VIPS_BANDFMT_SHORT:
			PRINT_INT(short, i);
			break;

		case VIPS_BANDFMT_UINT:
			PRINT_INT(unsigned int, i);
			break;

		case VIPS_BANDFMT_INT:
			PRINT_INT(int, i);
			break;

		case VIPS_BANDFMT_FLOAT:
			PRINT_FLOAT(float, i);
			break;

		case VIPS_BANDFMT_COMPLEX:
			vips_buf_appends(buf, "(");
		...
		case VIPS_BANDFMT_DOUBLE:
			PRINT_FLOAT(double, i);
			break;

		case VIPS_BANDFMT_DPCOMPLEX:
			vips_buf_appends(buf, "(");
...
imageinfo_from_rgb(Imageinfo *imageinfo, double *rgb)
{
	...
	if (im->Coding == VIPS_CODING_LABQ) {
	...
	else if (im->Coding == VIPS_CODING_NONE) {
		switch (im->Type) {
		...
		default:
			if (im_clip2fmt(in->im, out->im, VIPS_BANDFMT_FLOAT))
				return;
			break;
		}
	}
