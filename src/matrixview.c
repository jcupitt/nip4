...
matrixview_text_build(Matrixview *matrixview)
{
	...
	if (matrix->value.width > matrixview_max_width ||
		 ...
		 */
		width = VIPS_MIN(matrix->value.width, matrixview_max_width);
		height = VIPS_MIN(matrix->value.height, matrixview_max_height);
...
matrixview_refresh(vObject *vobject)
{
	...
	if (matrixview->display != matrix->display ||
		...
		/* Kill old UI stuff.
		 */
		VIPS_FREEF(gtk_widget_destroy, matrixview->sheet);
		VIPS_FREEF(gtk_widget_destroy, matrixview->table);
		VIPS_FREEF(gtk_widget_destroy, matrixview->swin);
		VIPS_FREEF(g_slist_free, matrixview->items);
		VIPS_FREEF(gtk_widget_destroy, matrixview->cbox);
